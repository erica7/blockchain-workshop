...
describe('BrewHttp', () => {
  ...
  it('should start an http server and build a block chain', () => {
    ...
    .then((brews) => {
      ...
      // TODO: Add some data to the block chain
  ...
  it('should start 2 different http servers and they should stay current with each other', () => {
    ...
    return rp(`http://localhost:${port1}/spawnBrew/${data1}`) // TODO: Add data1 to block chain
    .then( () => B.delay(messageDelay))
    .then(() => rp(`http://localhost:${port1}/addNode/${port2}`)) // TODO: Add server 2 as peer to server 1
    .delay(messageDelay)
    .then(() => rp(`http://localhost:${port1}/spawnBrew/${data2}`)) // TODO: Add data2 to block chain
    .delay(messageDelay)
